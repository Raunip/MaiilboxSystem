...
public class Connection
{
    ...
    public void dial(String key)
    {
        if (state == CONNECTED)
            ...
            messageMenu(key);
        else if (state == MESSAGE_MENU_FORWARD)
            messageForwardMenu(key);
        else if (state == MESSAGE_MENU_FORWARD_NUM)
            messageForwardNum(key);
    ...
    private void messageMenu(String key)
    {
        ...
        {
            state = MESSAGE_MENU_FORWARD;
            phone.speak(MESSAGE_MENU_TEXT_FORWARD);
        }
    }
    private void messageForwardMenu(String key) {
        if (key.equals("1")) {
            state = MESSAGE_MENU_FORWARD_NUM;
            phone.speak(FORWARD_MAILBOX_NUMBER);
        }
        else if (key.equals("2")){
            state = MAILBOX_MENU;
            phone.speak(MAILBOX_MENU_TEXT);
    ...
    private void messageForwardNum(String ext){
        forwardMailbox = system.findMailbox(ext);
        Message m = currentMailbox.getCurrentMessage();
        forwardMailbox.addMessage(m);
    }
    ...
    private  static final int MESSAGE_MENU_FORWARD =7;
    private  static final int MESSAGE_MENU_FORWARD_NUM =8;
    ...
    private static final String MESSAGE_MENU_TEXT =
         ...
         + "Enter 4 to return to the main menu\n"
                 + "Enter 5 to forward the current message";
    private static final String MESSAGE_MENU_TEXT_FORWARD =
            "Enter 1 to forward the current message\n"
            +"Enter 2 to return to the main menu";
    private  static final String FORWARD_MAILBOX_NUMBER =
            "Enter the Mailbox number to forward the message to.";
}
